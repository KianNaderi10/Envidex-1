...
import { fetchSpeciesDatabase, findSpeciesById, type SpeciesRecord } from "@/lib/species-api";
...
function getHabitatEmoji(habitat: string) {
  ...
  if (h.includes("grass") || h.includes("meadow") || h.includes("field") || h.includes("savanna")) return "🌾";
...
export default function CollectionPage() {
  ...
  const [allSpecies, setAllSpecies] = useState<SpeciesRecord[]>([]);
  const [loading, setLoading] = useState(true);
  const [exampleSpecies, setExampleSpecies] = useState<SpeciesRecord | null>(null);
  const [isExampleOpen, setIsExampleOpen] = useState(false);

  useEffect(() => {
    async function loadData() {
      try {
        const data = await fetchSpeciesDatabase();
        setAllSpecies(data);
        if (data.length > 0) {
          setExampleSpecies(data[0]);
        }
      } catch (error) {
  ...
  const collectedSpecies = useMemo(
    () =>
      collection
        .map((entry) => ({
          entry,
          species: findSpeciesById(allSpecies, entry.speciesId),
        }))
        .filter((item) => item.species !== undefined),
    [collection, allSpecies]
  );

  const collectedIds = new Set(collection.map((e) => e.speciesId));
  const completionPct =
    allSpecies.length > 0
      ? Math.round((collectedIds.size / allSpecies.length) * 100)
      : 0;

  const openExampleProfile = (species: SpeciesRecord) => {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <p className="text-sm text-muted-foreground">Loading field guide...</p>
      </div>
    );
  }
        ...
        <Card className="p-4 border-border/50 bg-card/60">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs font-bold text-primary">
              {collectedIds.size}/{allSpecies.length}
            </span>
          </div>
        ...
        <div className="px-4 flex-1 flex flex-col items-center justify-center gap-4 py-16">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Scan animals and plants around you to fill your Field Guide
            </p>
          </div>
          <Link
            href="/scan"
